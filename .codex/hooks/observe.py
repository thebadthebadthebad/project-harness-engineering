...
CONTENT_READ = re.compile(r"(?<![A-Za-z0-9_-])(?:cat|sed|head|tail|less|rg)(?![A-Za-z0-9_-])")
GLOB_OPTION = re.compile(r"(?:^|\s)(?:-g|--glob)(?:=|\s+)(?:'[^']*'|\"[^\"]*\"|\S+)")
...
def document_paths(command: str) -> list[str]:
    """Extract Markdown content-read paths while ignoring globs and path-only commands."""
    documents: list[str] = []
    for segment in re.split(r"&&|\|\||;|\n", command):
        if not CONTENT_READ.search(segment):
            continue
        if re.search(r"(?<![A-Za-z0-9_-])rg\s+[^;&|]*--files(?:\s|$)", segment):
            continue
        without_globs = GLOB_OPTION.sub(" ", segment)
        documents.extend(MARKDOWN_PATH.findall(without_globs))
...
def command_metadata(payload: dict[str, Any]) -> tuple[list[str], str | None]:
    ...
    if not isinstance(command, str):
        return [], None
    documents = document_paths(command)
