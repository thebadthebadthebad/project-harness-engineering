...
CANDIDATE = REPOSITORY / "tasks/implement-harness-stage-a/scripts"
PUBLIC_TEMPLATE = REPOSITORY / "project"
HARNESSCTL = CANDIDATE / "harnessctl.py"
...
class StageATest(unittest.TestCase):
    ...
    def candidate_template(self, name: str = "template") -> Path:
        ...
        shutil.copy2(CANDIDATE / "projectctl.py", target / "tools/projectctl.py")
        for source in (CANDIDATE / "project_harness").glob("*.py"):
            shutil.copy2(source, target / "tools/project_harness" / source.name)
        shutil.copy2(CANDIDATE / "template/.gitignore", target / ".gitignore")
        return target
