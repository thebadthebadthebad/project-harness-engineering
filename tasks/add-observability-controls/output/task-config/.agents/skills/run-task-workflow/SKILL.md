...
3. Perform the current Work Plan item within `TASK.md` Scope. Keep research, experiments, code, and temporary outputs in the Task locations named by the contract.
4. Update `STATUS.md` only when the current Work item changes. Keep exactly one `doing` item while active.
5. Validate changed code or evidence in proportion to the work. Do not treat format checks as a substitute for result interpretation.
