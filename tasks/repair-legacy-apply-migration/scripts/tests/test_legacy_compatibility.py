...
CANDIDATE = REPOSITORY / "tasks/repair-legacy-apply-migration/scripts"
...
class LegacyCompatibilityTest(unittest.TestCase):
    def setUp(self) -> None:
        ...
        shutil.copy2(CANDIDATE / "projectctl.py", self.template / "tools/projectctl.py")
        for source in (CANDIDATE / "project_harness").glob("*.py"):
            shutil.copy2(source, self.template / "tools/project_harness" / source.name)
        self.bundle = self.base / "bundle"
        self.command(
            "python3", str(CANDIDATE / "harnessctl.py"), "package",
    ...
    def test_install_precedes_semantic_migration_and_preserves_legacy_meaning(self) -> None:
        ...
        applied = self.command(
            "python3", str(CANDIDATE / "harnessctl.py"), "apply", str(root),
